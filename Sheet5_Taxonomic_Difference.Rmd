...
```{r, include = FALSE} 
library(ampvis2)
library(DESeq2) #analisis diferencia
library(phyloseq)
library(ggpubr)
```
...
otutable <- data.frame(OTU = rownames(phyloseq::otu_table(obj)@.Data),
                       ...
                       check.names = FALSE
) #Especificar una tabla para poder acceder a los datos
metadata <- data.frame(phyloseq::sample_data(obj), 
                       check.names = FALSE
) #Se arma un nuevo data frame
...
av2 <- amp_load(otutable, metadata) #herramientas para los analisis
```
...
```{r, warning = FALSE, fig.align='center'}
#gráficos con estadisticos que nos permiten ver si sí hay diferencias (Analisis diferencial)
#Util para transcriptómica y single-cells-genomic
diagdds = phyloseq_to_deseq2(psd5, ~Treatment)
gm_mean = function(x, na.rm=TRUE){
  exp(sum(log(x[x > 0]), na.rm=na.rm) / length(x))
}
geoMeans = apply(counts(diagdds), 1, gm_mean) 
diagdds = estimateSizeFactors(diagdds, geoMeans = geoMeans)#corregir la longitus de las muestras/normalizar los datos

diagdds = DESeq(diagdds, test="Wald", fitType="local") #Tomamos el objeto de phyloseq y lo transformamos para analizarlo con DEseq (Comparar cosas entre si)
#Se aplica la prueba paramétrica
#La consola muestra los pasos que hace y estadisticos que saca, se pueden modificar (tutorial)

res = results(diagdds, cooksCutoff = FALSE) #Obtención de resultados
