...
```{r}
alpha_pd <- estimate_pd(psd5) # estimar la diversidad filogenetica para cada una de las muestras. Relaciones evolutivas entre las especies
data <- cbind(sample_data(psd5), alpha_pd) #Unir las estimaciones de diversidad con los datos de metadata (las demás variables). 
psd5.anova <- aov(PD ~ Treatment, data) #Comparar tratamiento/variables de interes
psd5.anova.table <- xtable(psd5.anova) #tabla de las variables y sus estadisticos
psd5.anova.table
tab <- microbiome::alpha(psd5)
head(tab)
#Crear y mostrar los estadisticos de diversidad. La ventaja de usar el paquete esq nos permite obtener todos los estadístics sin tener que ir una por una
```

## STRUCTURE OF MICROBIAL CUMMUNITIES

```{r, include=FALSE}
tab <- richness(psd5) #Se analiza uno por uno los estadisticos de los datos
tab <- dominance(psd5, index = "all")
tab <- rarity(psd5, index = "all")
tab <- microbiome::coverage(psd5, threshold = 0.5)
tab <- inequality(psd5)
tab <- evenness(psd5, "all") #como se distribuyen los ind entre las spp
```

## DIVERSITY INDEXES

```{r, include=FALSE}
psd5.2 <- prune_taxa(taxa_sums(psd5) > 0, psd5) #Eliminar muestras con estadísticos que sean iguales a 0
...
#print(pShannon)#Gráfico con estadística
