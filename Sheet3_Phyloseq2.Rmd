...
#En TIME busca eestos dos tiempos en particular
psd1_actino <- subset_taxa(ps1, Phylum == "Actinobacteriota")
#Generar un subset de un filo
```
...
#El ! significa que no este en la columna XXX
```
...
#Cargan los códigos para graficar desde github. Otro tipo de gráficos
```
...
``` #La gráfica nos muestra como va aumentando el # de especies (Acumulación de spp)

## PHYLOGENY TREE GRAPH

```{r}
plot_tree(psd5, method = "treeonly", ladderize = "left")
#figura del árbol filogenético
```

```{r}
df <- psmelt(psd5)
df
#tabla de muestra, OTU correspondiente, la abundancia de dicho grupo y otras cositas
```
